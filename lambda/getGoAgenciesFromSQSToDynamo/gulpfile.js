...
gulp.task('installDependencies', () =>
  gulp.src('publish/package.json').pipe(install({ production: true }))
);

gulp.task('fixRequires', () =>
  gulp
...
gulp.task('createZip', () =>
  gulp.src('publish/**/*', { dot: true }).pipe(zip.dest('publish/archive.zip'))
);
